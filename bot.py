        ...
        while not entry["stop"]:
            try:
                ...
                cl = TelegramClient(StringSession(session_data), config.API_ID, config.API_HASH,
                    connection_retries=5, retry_delay=3, timeout=30, auto_reconnect=True,
                    flood_sleep_threshold=60, device_model="AMEL SELF55", system_version="1.0.0", app_version="1.2.0")
                entry["client"] = cl
                ...
                is_now_owner = (me.id == config.OWNER_TG_ID or
...
def _register_handlers(cl: TelegramClient, owner_id: int, entry: dict):
    ...
    async def on_incoming(event):
        ...
        if not event.is_private and not is_tagged:
            if db.get_setting(owner_id, "auto_seen_active") == "1":
                try: await cl.send_read_acknowledge(chat_id, msg)
                except: pass
            ...
            if db.get_setting(owner_id, "enemy_reply_active") == "1" and db.is_enemy(owner_id, sender_id):
                try: await event.reply(random.choice(ENEMY_REPLIES))
                except: pass
                ...
                if challenge and not challenge.get('solved') and replied.id == challenge['message_id']:
                    ...
                    if user_answer == challenge['correct_answer']:
                        ...
                        if account:
                            ...
                            await event.reply(f"🎉 **تبریک!** @{sender.username or sender.first_name}\n✅ پاسخ صحیح! ۱ الماس به حساب شما اضافه شد.")
                            db.solve_math_challenge(challenge['id'])
                        else:
                            await event.reply(f"❌ شما در پنل ثبت‌نام نکرده‌اید!\nلطفاً ابتدا در ربات ثبت‌نام کنید.")
    ...
    async def on_outgoing(event):
        ...
        if text == "سلف خاموش":
            ...
            return
        config_commands = [
            "منشی روشن", "منشی خاموش", "پیام منشی", "ضد حذف روشن", "ضد حذف خاموش",
            "ضد لینک روشن", "ضد لینک خاموش", "قفل پیوی روشن", "قفل پیوی خاموش",
            "سین خودکار روشن", "سین خودکار خاموش", "ری‌اکشن روشن", "ری‌اکشن خاموش",
            "ذخیره مدیا روشن", "ذخیره مدیا خاموش", "ساعت نام روشن", "ساعت نام خاموش",
            "ساعت بیو روشن", "ساعت بیو خاموش", "پاسخ دشمن روشن", "پاسخ دشمن خاموش",
...
async def _handle_command(cl, event, text, owner_id, entry):
    msg = event.message
    def gs(key, default=None): return db.get_setting(owner_id, key, default)
    def ss(key, value): db.set_setting(owner_id, key, value)
    async def edit(t): await _safe_edit(event, owner_id, t)
    ...
    if text.startswith("تنظیم دشمن"):
        ...
        else: await edit("❗ روی پیام کاربر ریپلای کن یا آیدی عددی بنویس.")
        ...
        if not enemies: await edit("📋 لیست دشمن خالی است.")
        else:
            lines = [f"🔴 لیست دشمن ({len(enemies)} نفر):\n"]
            for e in enemies: lines.append(f"• {e['name'] or e['username'] or e['user_id']} — `{e['user_id']}`")
            await edit("\n".join(lines))
        ...
        if not friends: await edit("📋 لیست دوست خالی است.")
        else:
            lines = [f"💚 لیست دوست ({len(friends)} نفر):\n"]
            for f in friends: lines.append(f"• {f['name'] or f['username'] or f['user_id']} — `{f['user_id']}`")
            await edit("\n".join(lines))
    elif text == "پاک کردن لیست دوست":
        db.clear_friends(owner_id)
        await edit("🗑️ لیست دوست پاک شد.")
    elif text == "منشی روشن": ss("secretary_active", "1"); await edit("🤖 منشی خودکار روشن شد.\n💡 هر کاربر فقط هر 24 ساعت یک بار پاسخ می‌گیرد.")
    elif text == "منشی خاموش": ss("secretary_active", "0"); await edit("🤖 منشی خودکار خاموش شد.")
    elif text.startswith("پیام منشی "): ss("secretary_message", text[len("پیام منشی "):].strip()); await edit("✅ پیام منشی تنظیم شد.")
    elif text == "ضد حذف روشن": ss("anti_delete_active", "1"); await edit("🛡️ ضد حذف روشن شد.")
    elif text == "ضد حذف خاموش": ss("anti_delete_active", "0"); await edit("🛡️ ضد حذف خاموش شد.")
    elif text == "ضد لینک روشن": ss("anti_link_active", "1"); await edit("🔗 ضد لینک روشن شد.")
    elif text == "ضد لینک خاموش": ss("anti_link_active", "0"); await edit("🔗 ضد لینک خاموش شد.")
    elif text == "قفل پیوی روشن": ss("private_lock_active", "1"); await edit("🔒 قفل پیوی روشن شد.")
    elif text == "قفل پیوی خاموش": ss("private_lock_active", "0"); await edit("🔓 قفل پیوی خاموش شد.")
    elif text == "سین خودکار روشن": ss("auto_seen_active", "1"); await edit("👁️ سین خودکار روشن شد.")
    elif text == "سین خودکار خاموش": ss("auto_seen_active", "0"); await edit("👁️ سین خودکار خاموش شد.")
    elif text == "ری‌اکشن روشن": ss("auto_reaction_active", "1"); await edit("❤️ ری‌اکشن خودکار روشن شد.")
    elif text == "ری‌اکشن خاموش": ss("auto_reaction_active", "0"); await edit("❤️ ری‌اکشن خودکار خاموش شد.")
    elif text.startswith("ری‌اکشن "): emoji = text[len("ری‌اکشن "):].strip(); ss("auto_reaction_emoji", emoji); await edit(f"✅ ری‌اکشن پیش‌فرض: {emoji}")
    elif text == "ذخیره مدیا روشن": os.makedirs(f"saved_media/{owner_id}", exist_ok=True); ss("auto_save_media", "1"); await edit("💾 ذخیره خودکار مدیا روشن شد.")
    elif text == "ذخیره مدیا خاموش": ss("auto_save_media", "0"); await edit("💾 ذخیره خودکار مدیا خاموش شد.")
        ...
        if not channel_input: await edit("❗ فرمت: سیو کانال [لینک یا آیدی] [تعداد اختیاری]")
        else:
    ...
    elif text == "توقف سیو": ss("channel_save_active", "0"); await edit("🛑 سیو کانال متوقف شد.")
    elif text == "سایلنت چت روشن": chat = await event.get_chat(); db.add_silent_chat(owner_id, chat.id); await edit("🔇 این چت سایلنت شد.")
    elif text == "سایلنت چت خاموش": chat = await event.get_chat(); db.remove_silent_chat(owner_id, chat.id); await edit("🔔 سایلنت این چت برداشته شد.")
    elif text.startswith("سایلنت کاربر "): uid = int(text.split()[-1]); db.add_silent_user(owner_id, uid); await edit(f"🔇 کاربر {uid} سایلنت شد.")
    elif text.startswith("لغو سایلنت کاربر "): uid = int(text.split()[-1]); db.remove_silent_user(owner_id, uid); await edit(f"🔔 سایلنت کاربر {uid} برداشته شد.")
    elif text == "پاسخ دشمن روشن": ss("enemy_reply_active", "1"); await edit("⚔️ پاسخ خودکار به دشمن روشن شد.")
    elif text == "پاسخ دشمن خاموش": ss("enemy_reply_active", "0"); await edit("⚔️ پاسخ خودکار به دشمن خاموش شد.")
        ...
        await edit("\n".join(lines))
    elif text == "ساعت نام روشن": ss("clock_name_active", "1"); await edit("⏰ ساعت در نام روشن شد.\n💡 فونت فعلی روی ساعت اعمال می‌شود.")
    elif text == "ساعت نام خاموش": ss("clock_name_active", "0"); await edit("⏰ ساعت در نام خاموش شد.")
    elif text == "ساعت بیو روشن": ss("clock_bio_active", "1"); await edit("⏰ ساعت در بیو روشن شد.\n💡 فونت فعلی روی ساعت اعمال می‌شود.")
    elif text == "ساعت بیو خاموش": ss("clock_bio_active", "0"); await edit("⏰ ساعت در بیو خاموش شد.")
        ...
        else: await edit("❗ فرمت: اسپم [تعداد] [متن]")
    elif text == "توقف اسپم": ss("spam_active", "0"); await edit("🛑 اسپم متوقف شد.")
        ...
        if len(parts) >= 2 and parts[1].isdigit():
            ...
            if 1 <= slot <= 10:
                ...
                else: await edit("❗ روی پیام مورد نظر ریپلای کن.")
            else: await edit("❗ اسلات باید بین ۱ تا ۱۰ باشد.")
        ...
        if len(parts) >= 3 and parts[2].isdigit():
            ...
            if saved:
                ...
                await msg.delete()
            else: await edit(f"❗ اسلات {slot} خالی است.")
    elif text.startswith("ترجمه "):
        to_tr = text[len("ترجمه "):].strip()
        if not to_tr:
            replied = await event.get_reply_message()
            if replied: to_tr = replied.text or ""
        if to_tr: await edit(f"🌐 ترجمه:\n{await _translate(to_tr)}")
        else: await edit("❗ متن یا ریپلای لازم است.")
    elif text.startswith("هوا "): await edit(await _get_weather(text[len("هوا "):].strip()))
    elif text in ("قیمت دلار", "ارز"): await edit(await _get_currency())
    elif text == "وضعیت":
        ...
        await edit("\n".join(lines))
    elif text in ("راهنما", "help"): await edit(_help_text())
        ...
        else: await edit("❗ فرمت: ارسال زمان‌بندی [YYYY-MM-DD HH:MM] متن")
...
async def _resolve_target(event, parts):
    ...
    if replied:
        ...
        if sender:
            return {"id": sender.id, "username": getattr(sender, "username", None), "name": getattr(sender, "first_name", str(sender.id))}
    for p in parts[1:]:
        if p.lstrip("-").isdigit(): return {"id": int(p), "username": None, "name": p}
    return None

async def _do_spam(cl, owner_id, chat_id, text, count):
    delay = float(db.get_setting(owner_id, "spam_delay", "2"))
    for _ in range(count):
        if db.get_setting(owner_id, "spam_active") != "1": break
        try:
            await cl.send_message(chat_id, text)
            await asyncio.sleep(delay)
        except FloodWaitError as e: await asyncio.sleep(min(e.seconds + 1, 60))
        except: break
...
async def _save_channel_media(cl, channel_input, limit, owner_id):
    ...
    try:
        me = await cl.get_me()
        if channel_input.startswith("https://t.me/"): channel_input = channel_input.replace("https://t.me/", "")
        if channel_input.startswith("@"): channel_input = channel_input[1:]
        saved = skipped = 0
        async for msg in cl.iter_messages(channel_input, limit=limit):
            if db.get_setting(owner_id, "channel_save_active") != "1": break
            if msg.media:
                try:
                    path = await cl.download_media(msg, file=media_dir + "/")
                    if path:
                        caption = f"📥 سیو کانال\n📌 پیام #{msg.id}"
                        if msg.text: caption += f"\n📝 {msg.text[:100]}"
                        await cl.send_file(me.id, path, caption=caption)
                        saved += 1
                        await asyncio.sleep(0.1)
                except FloodWaitError as e: await asyncio.sleep(min(e.seconds + 2, 60))
                except: skipped += 1
            else: skipped += 1
...
async def _get_weather(city):
    ...
    except: return "⚠️ خطا در دریافت اطلاعات هوا"
...
async def _get_currency():
    try:
        ...
        with urllib.request.urlopen("https://api.exchangerate-api.com/v4/latest/USD", timeout=5) as resp:
            data = json.loads(resp.read().decode())
            r = data["rates"]
            return (f"💵 نرخ ارز:\nدلار/یورو: {round(1/r['EUR'],4)}\nدلار/پوند: {round(1/r['GBP'],4)}\nدلار/روبل: {round(r['RUB'],2)}")
    except: return "⚠️ خطا در دریافت قیمت ارز"
        ...
        try:
            ...
            if current_minute != last_minute:
                ...
                if db.get_setting(owner_id, "clock_name_active") == "1":
                    ...
                    except Exception as e: logger.warning(f"❌ خطا در به‌روزرسانی نام: {e}")
                if db.get_setting(owner_id, "clock_bio_active") == "1":
                    ...
                    except Exception as e: logger.warning(f"❌ خطا در به‌روزرسانی بیو: {e}")
            await asyncio.sleep(5)
        except Exception as e:
    ...
    while True:
        try:
            ...
            if op == '+':
                ...
                answer = str(a * b)
                question = f"{a} × {b} = ?"
            msg = await cl.send_message(CHAT_ID,
                f"🧮 **چالش ریاضی!**\n\n❓ {question}\n\n⏱️ اولین نفر با پاسخ صحیح برنده ۱ الماس می‌شود!\n📝 پاسخ را به صورت عدد لاتین ریپلای کنید.")
            db.create_math_challenge(owner_id, question, answer, CHAT_ID, msg.id)
            await asyncio.sleep(7200)
            challenge = db.get_math_challenge(owner_id)
            if challenge and not challenge.get('solved'):
                await cl.send_message(CHAT_ID, f"⏰ زمان چالش ریاضی به پایان رسید!\nپاسخ صحیح: `{answer}`")
                db.solve_math_challenge(challenge['id'])
        except Exception as e:
